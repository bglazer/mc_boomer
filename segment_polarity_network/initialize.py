...
def allActions():
    ...
    for src,dst in internal_rules:
        actions.add(Action(srcs=(MultiSource(span='i', src=src),), type='a', dst=dst))
        actions.add(Action(srcs=(MultiSource(span='i', src=src),), type='i', dst=dst))
    ...
    for src,dst in external_rules:
        actions.add(Action(srcs=(MultiSource(span='e', src=src),), type='a', dst=dst))
        actions.add(Action(srcs=(MultiSource(span='e', src=src),), type='i', dst=dst))
    ...
    compounds = [(MultiSource('CIA','i'), MultiSource('SLP','i')), 
                 (MultiSource('CI','i'),  MultiSource('SMO','i')),
                 (MultiSource('CI','i'),  MultiSource('HH','e')),
                 ]
                 # Already set as an action below
                 #(MultiSource('PTC','i'), MultiSource('HH','e')),
    ...
    wg_actions = [
                 ...
                 Action(srcs=(MultiSource('CIA','i'), MultiSource('wg','i')), dst='wg', type='i'),
                 Action(srcs=(MultiSource('SLP','i'), MultiSource('wg','i')), dst='wg', type='a'),
                 Action(srcs=(MultiSource('SLP','i'), MultiSource('wg','i')), dst='wg', type='i'),
                 ]
...
def initialState():
    ...
    gene_protein_rules = [Action(srcs=(MultiSource(span='i',src='en'),), type='a', dst='EN'),
                          Action(srcs=(MultiSource(span='i',src='wg'),), type='a', dst='WG'),
                          Action(srcs=(MultiSource(span='i',src='ci'),), type='a', dst='CI'),
                          Action(srcs=(MultiSource(span='i',src='ptc'),), type='a',dst='PTC'),
                          Action(srcs=(MultiSource(span='i',src='hh'),), type='a', dst= 'HH')]

    # Initialize CI to activate CIA and CIR, because these are truncated CI protein
    ci_rules = [Action(srcs=(MultiSource(span='i',src='CI'),), type='a', dst='CIA'),
                Action(srcs=(MultiSource(span='i',src='CI'),), type='a', dst='CIR')]
    ...
    ph_rule = [Action(srcs=(MultiSource('PTC','i'), MultiSource('HH','e')), dst ='PH', type='a')]
    ...
    # space. 
    ptc_rule = [Action(srcs=(MultiSource('PTC','i'), MultiSource('~HH','e')), dst='PTC', type='a')]
    smo_rule = [Action(srcs=(MultiSource('~PTC','i'),), dst='SMO', type='a')]
    ...
    external_internal_rules = [
        Action(srcs=(MultiSource(src='HH', span='e'),), dst='CIR', type='i'),
        Action(srcs=(MultiSource(src='WG', span='e'),), dst='en', type='a')
    ]
