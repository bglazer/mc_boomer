...
def action_set(rules, cell=None):
    actions = set()
    for dst, (act,inh) in rules.items():
        if cell is None or cell==dst[1]:
            for src in act:
                actions.add(to_action(tuple(src),dst,'a'))
            for src in inh:
                actions.add(to_action(tuple(src),dst,'i'))
    return actions

def to_action(src, dst, itype):
    dst_node, dst_cell = dst
    rev = []
    for clause in src:
        src_node, src_cell = clause
        if src_cell != dst_cell:
            extent = 'e'
        else:
            extent = 'i'
        rev.append((src_node, extent))
    return Action(dst=dst_node, srcs=tuple(rev), type=itype)
...
#The first two columns contain the node indices which are joined in each step. 
#The input nodes are labeled 0,..., N−1, and the newly generated nodes have the labels N,...,2N−2.
#The third column contains the distance between the two nodes at each step, 
#   ie. the current minimal distance at the time of the merge. 
#The fourth column counts the number of points which comprise each new node
def reorder_hierarchical(r):
    clusters = {}
    N = r.shape[0]
    i=0
    cluster_idx = N+1
    ...
    for row in r:
        ...
        a = int(a)
        b = int(b)
        # a <= N indicates that a is an index of a data-point, rather than the  
        # index of aggregated points (i.e. a cluster)
        if a <= N and b <= N:
            clusters[cluster_idx] = [a,b]
        if a <= N and b > N:
            clusters[cluster_idx] = clusters[b] + [a]
        if a > N and b <= N:
            clusters[cluster_idx] = clusters[a] + [b]
        if a > N and b > N:
            clusters[cluster_idx] = clusters[b] + clusters[a]
        cluster_idx+=1
    idx = clusters[cluster_idx-1]
    return idx
...
def format_actions(actions, end='\n'):
    for i,action in enumerate(actions):
        ...
        print(f'({" and ".join(exprs)} {t} {action.dst}) {more}{expr_end}', end='')
...
def print_attractors(attractors, truth, squashed=False, num_cells=4, latex=False):
    if latex:
...
def read_models(models_filename, rewards=True):
    if gzipped:
        open = gzip.open
